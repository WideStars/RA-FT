...
def get_arguments():
    ...
    parser.add_argument('--lambda2', default=1.0, type=float, help='weight for the Con loss')
    parser.add_argument('--tau1', default=2.0, type=float, help='temperature for the C loss')
    parser.add_argument('--tau2', default=1.5, type=float, help='temperature for the B loss')
...
def main():
    ...
    text_feats = clip_classifier(feat_path, train_set.classnames, gpt3_prompt, clip_model)
    ...
    trainer = Trainer(cfg, clip_model, my_pooling, train_loader, val_loader, logger, text_feats)
    trainer.train()
